...
resource "aws_kms_key" "media_key" {
  description             = "KMS Key for Our Perfect Wedding media asset encryption"
  deletion_window_in_days = 30
  enable_key_rotation     = true

  tags = {
    Name = "our_perfect_wedding_production_workflow-kms-key"
  }
}

# 2. S3 Media Vault Bucket
resource "aws_s3_bucket" "media_vault" {
  bucket        = "nwhite-our-perfect-wedding-production-workflow-${var.environment}"
  force_destroy = false
}
...
resource "aws_iam_role" "post_production_role" {
  name = "our_perfect_wedding_production_workflow-post-production-role"
...
resource "aws_iam_policy" "post_production_policy" {
  name        = "our_perfect_wedding_production_workflow-post-production-policy"
  description = "Scoped S3 and KMS permissions for Our Perfect Wedding post-production workflow"
...
resource "aws_cloudwatch_log_group" "workflow_logs" {
  name              = "/aws/media-pipeline/our_perfect_wedding_production_workflow"
